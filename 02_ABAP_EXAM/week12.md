...
DATA: it_join TYPE STANDARD TABLE OF t_join INITIAL SIZE 0,
      wa_join TYPE t_join.

DATA: it_collect TYPE STANDARD TABLE OF t_join INITIAL SIZE 0,
...
*&      Form  DATA_RETRIEVAL
*&---------------------------------------------------------------------*
*       Retrieve data form SBOOK SCUSTOM SCARR table and populate itab it_move
*----------------------------------------------------------------------*
FORM data_retrieval.
  ...
  IF r1 = 'X'.
    ...
    LOOP AT it_join INTO wa_join.
      COLLECT wa_join INTO it_collect.
    ENDLOOP.

    LOOP AT it_collect INTO wa_collect.           " it_collect 테이블 구조 변수를 반복
      REFRESH t_colinfo_table.                    "Coloring에 대한 정보를 담을 internal table
      CLEAR: wa_move, wa_join, t_colinfo_table.   " clear
      wa_move-days = wa_collect-fldate - wa_collect-order_date. " 날짜 계산
      MOVE-CORRESPONDING wa_collect TO wa_move.   " 한 행을 읽어와서 wa_move 행 구조 변수에 넣는다
  ...
  ELSEIF r2 = 'X'.
    ...
    LOOP AT it_collect INTO wa_collect.           " it_collect 테이블 구조 변수를 반복
      ...
      wa_move-days = wa_collect-fldate - wa_collect-order_date.
      MOVE-CORRESPONDING wa_collect TO wa_move.   " 한 행을 읽어와서 wa_move 행 구조 변수에 넣는다

...
FORM upload_from_excel .
  ...
  lv_numberofcolumns = 3 .                    " 매핑할 컬럼 개수
  ...
  " <gt_data>에서 한 행씩 읽어서 <ls_data>에 넣는다.
    ...
    DO lv_numberofcolumns TIMES.                  " 컬럼 개수 만큼 반복(3번)
      ...
      IF sy-subrc = 0 .
        CASE sy-index .
          ...
          when 2 .                            " sy-index가 2이면
            gs_EXCEL-EMPNO = <lv_field>.      " 두 번재 열의 값을 EMPNO
          when 3 .                            " sy-index가 2이면
            gs_EXCEL-ENAME = <lv_field>.      " 세 번재 열의 값을 ENAME
          WHEN OTHERS.
*            WRITE : <lv_field> .
        ENDCASE .
      ENDIF.
...
FORM get_zsc_data .
  LOOP AT GT_EXCEL INTO GS_EXCEL.             " 엑셀 데이터를 담은 gt_excel 반복
    ...
    " 1. EMPNO (Primary Key) 키 검증
    IF GS_ZSC-EMPNO IS INITIAL.              " 필수 키값이 누락된 경우
      GS_ZSC-ZSTATUS = ICON_LED_RED.
      GS_ZSC-ZRESULT = GS_ZSC-ZRESULT && '[CARRID 키값이 없습니다.]'.
    ELSE.     " 중복 체크 (GT_ZEMP와 비교)
      SORT GT_ZEMP BY EMPNO.

      " GT_ZEMP 테이블에서 데이터를 한 줄 읽어서 GS_ZEMP라는 구조체(Work Area)에 넣기.
      " 검색 조건에 맞는것을 넣는다. EMPNO 필드 값이 현재 엑셀 줄의 EMPNO와 같은 것.
      " BINARY SEARCH 를 사용한다. 그래서 위에서 미리 정렬을 해야한다.
      READ TABLE GT_ZEMP INTO GS_ZEMP
                               WITH KEY EMPNO = GS_ZSC-EMPNO
                               BINARY SEARCH.

      IF SY-SUBRC = 0.                      " 이미 DB에 존재하는 키값이면 에러 처리.
             GS_ZSC-ZSTATUS = ICON_LED_RED.
             GS_ZSC-ZRESULT = GS_ZSC-ZRESULT && '[EMPNO 키값이 이미 들어있습니다.]'.
      ENDIF.
    ENDIF.
...
FORM save_zsc_final  CHANGING GS_ZSC LIKE GS_ZSC.
  ...
  IF GS_ZSC-ZSTATUS = ICON_LED_YELLOW.
    MOVE-CORRESPONDING GS_ZSC TO GS_ZEMP.
    GS_ZEMP-MANDT = SY-MANDT.

    INSERT INTO ZEMP VALUES GS_ZEMP.

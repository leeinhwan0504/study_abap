...
DATA: fcode    TYPE TABLE OF sy-ucomm,    " 업로드 모드일때 버튼 코드 안나오게 하려고(EDIT, DEL 버튼이 업로드화면에서는 안나오게 하는 구조)
      wa_fcode TYPE sy-ucomm.
*&---------------------------------------------------------------------*

DATA: it_zscarrcp TYPE TABLE OF ZSCARR WITH HEADER LINE,    " 편집 전과 후를 비교하려고 복사해두는 테이블
      it_changes  TYPE TABLE OF ZSCARR WITH HEADER LINE,    " 변경한것만 저장하려고
      it_deletes  TYPE TABLE OF ZSCARR WITH HEADER LINE.    " 삭제한것만 저장하려고
```
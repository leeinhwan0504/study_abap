...
*  lt_clip = 'SCARR 테이블 엑셀 샘플양식'.
  ...
  CONCATENATE 'MANDT' 'CARRID' 'CARRNAME' 'CURRCODE' 'URL' INTO lt_clip SEPARATED BY lv_deli.
  APPEND lt_clip.
  CLEAR lt_clip.
...
* 입력 데이터 점검을 위해 사용할 DB 데이터
  ...
  IF r2 = 'X'.
    it_zscarrcp[] = GT_ZSCARR[].    " 편집전과 후를 비교하려고 it_zscarrcp 테이블 구조에 복사해둔다.
  ENDIF.
...
*   go_grid->register_edit_event( i_event_id = cl_gui_alv_grid=>mc_evt_enter ).
    ...
    CALL METHOD go_grid->set_ready_for_input
            EXPORTING
              i_ready_for_input = 0.    " 시작해서 바로 편집모드가 안되게 하려고 0으로 설정해서 조회모드로 먼저 보이게
...
*          PERFORM refresh_data.    " 주석처리해도 refresh 된다.
...
*  LOOP AT gt_zscarr INTO gs_zscarr WHERE box = 'X'.

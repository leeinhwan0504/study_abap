...
* 다운로드 양식 선택
    " DATA: LS_KEY LIKE WWWDATATAB. 이렇게 선언했었다. 바이너리 파일(예: Excel 템플릿) 을 가져올 때 사용되는 키 정보를 저장하는 로컬 구조체 변수
    LS_KEY-OBJID = 'ZTEST14_EXCEL01'. " 다운로드할 객체의 ID를 지정. 'ZTEST14_EXCEL01'이라는 이름의 Excel 파일(템플릿)을 다운로드하려 합니다.
    LS_KEY-RELID = 'MI'.              " 객체가 저장된 릴레이션 ID를 지정합니다. 'MI'는 보통 MIME 객체를 나타냅니다.

* 파일 경로 조회
  " LS_KEY-OBJID를 인자로 넣어서 SET_DIRECTORY 함수 호출.
  PERFORM SET_DIRECTORY USING LS_KEY-OBJID. " 팝업띄워서 저장할 경로 선택. 다운로드할 파일의 저장 위치(디렉토리) 및 기본 파일명을 설정.

* 엑셀 다운
  PERFORM DOWNLOAD_EXCEL_SMPL USING LS_KEY-OBJID. " 엑셀 틀어서 파일 내용 채워주고 저장
...
FORM set_directory  USING    p_ls_key_objid.
  ...
  OBJFILE->DIRECTORY_BROWSE( EXPORTING  INITIAL_FOLDER  = GV_INITIAL_DIR
                             ...
                             EXCEPTIONS CNTL_ERROR      = 1
                                        ...
                                        NOT_SUPPORTED_BY_GUI = 3 ).
  IF SY-SUBRC = 0.
    GV_FILE = GV_DIRECTORY && '\' && LS_KEY-OBJID && '.xlsx'.   " 전체 경로
  ELSE.
...
* OLE OBJECT 생성 & 실행
  CREATE OBJECT GO_APPLICATION 'Excel.Application'. " 엑셀 실행

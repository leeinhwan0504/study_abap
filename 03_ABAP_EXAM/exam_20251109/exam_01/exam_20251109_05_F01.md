...
* OLE OBJECT 생성 & 실행
  ...
  CREATE OBJECT GO_APPLICATION 'Excel.Application'. " 엑셀을 실행한다.

* 화면 DISPLAY 설정 (1을 설정하면 DISPLAY)
  " GO_APPLICATION 객체의 Visible 속성을 1로 지정하면 현재 실행중인 엑셀이 보여이게된다.
  SET PROPERTY OF GO_APPLICATION 'Visible' = 1.     " 엘셀이 열리면서 저장
...
* WORKBOOK 및 WORKBOOK 설정 & OPEN
  ...
  CALL METHOD OF GO_WBOOK 'Add'.  " 얻어낸 통합 문서 컬렉션 객체(GO_WBOOK)의 Add 메소드를 실행. GO_WBOOK 객체에 빈 통합 문서가 추가된다.
...
* 최초 실행 SHEET는 첫번째
  ...
  SET PROPERTY OF GO_SHEET 'Name' = 'ZSCARR'.       " 시트 이름 설정

  " GO_APPLICATION 객체의 ActiveWorkbook 속성 값을 읽어온다.
  " 현재 사용자에게 활성화되어 있는 통합 문서(Workbook) 객체가 속성 값이다. GO_WBOOK 객체에 저장.
  GET PROPERTY OF GO_APPLICATION 'ActiveWorkbook' = GO_WBOOK.   " 통합문서 활성화

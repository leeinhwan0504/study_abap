...
* 업로드 조건에 따라 ALV 출력 데이터 취합
  PERFORM GET_ZSC_DATA.       " 여기 클릭해서 수정. 수정19 수정20 수정21 수정22
...
*                                             13 'CURRENCY'.
  ...
  DATA : TAB1 TYPE TABLE OF SPFLI WITH HEADER LINE.   " 수정14 SPFLI 로 할 것
  DATA : RNUM TYPE I.
  DATA : CNUM TYPE I.
  ...
  SELECT * FROM ZSPFLI INTO TABLE TAB1.   " 수정15
*     UP TO 2 ROWS.
...
*        lt_clip = <tfield>.
*      ELSE.
*        CONCATENATE  lt_clip <tfield> INTO lt_clip SEPARATED BY lv_deli.
*      ENDIF.
...
**                TAB1-URL TAB1-LANGU TAB1-CURRENCY INTO lt_clip SEPARATED BY lv_deli.
*    ENDDO.
*
*    APPEND lt_clip.
*    CLEAR lt_clip.
...
* 입력 데이터 점검을 위해 사용할 DB 데이터
  SELECT *
    ...
    SORT GT_TABLE BY CARRID CONNID.                 " 수정17 수정18
...
FORM get_zsc_data .
  ...
  LOOP AT GT_EXCEL INTO GS_EXCEL.
    ...
    IF GS_ZSC-CARRID IS INITIAL OR GS_ZSC-CONNID IS INITIAL.    " 수정19
      GS_ZSC-ZSTATUS = ICON_LED_RED.
      GS_ZSC-ZRESULT = GS_ZSC-ZRESULT && '[키값이 없습니다.]'.
    ELSE.
      SORT GT_TABLE BY CARRID CONNID.                           " 수정20

      READ TABLE GT_TABLE INTO GS_TABLE
                           WITH KEY CARRID = GS_ZSC-CONNID      " 수정21
...
*ENDIF.
  ...
  LOOP AT GT_FCAT INTO GS_FCAT WHERE FIELDNAME = 'CONNID'.  " 수정28
    GS_FCAT-LZERO   = 'X'.
    MODIFY GT_FCAT FROM GS_FCAT.
  ENDLOOP.

  " 여기는 다하고 마지막에 하기. 수정39
  LOOP AT GT_FCAT INTO GS_FCAT WHERE FIELDNAME = 'FLTIME'.
    GS_FCAT-EDIT_MASK = '__________'.
    MODIFY GT_FCAT FROM GS_FCAT.
  ENDLOOP.
...
FORM save_zsc_data .
    LOOP AT GT_ZSC ASSIGNING FIELD-SYMBOL(<FS_ZSC>).
      PERFORM SAVE_ZSC_FINAL CHANGING <FS_ZSC>.       " 여기서 부터 클릭하면서 수정. " 수정29
    ENDLOOP.
...
IF GS_ZSC-ZSTATUS = ICON_LED_YELLOW.
     ...
     INSERT INTO ZSPFLI VALUES GS_TABLE.        " 수정29
...
* 입력 데이터 점검을 위해 사용할 DB 데이터
  SELECT *
    FROM ZSPFLI
    INTO CORRESPONDING FIELDS OF TABLE GT_TABLE.      " 수정23
...
*      IF togl = 'X'.
       IF go_grid->is_ready_for_input( ) = 1.
        ...
        IF it_cp[] NE gt_table[].
          ...
          IF p_confirm = '1'.
            PERFORM f_save_data.      " 여기 클릭해서 수정. 수정30 31 32
...
*&---------------------------------------------------------------------*
FORM f_save_data .
  DATA: wa_cp   TYPE ZSPFLI,    " 수정30
        wa_tmp  TYPE ZSPFLI.    " 수정31
  ...
  LOOP AT gt_table INTO gs_table.
    ...
    IF wa_cp NE gs_table.
      ...
      MODIFY ZSPFLI FROM wa_tmp.  " 수정32

...
*& <--  p2        text
*&---------------------------------------------------------------------*
FORM set_function_key .   " 기능 키 버튼을 만드는 함수
*SMPL
  G_FUNCTION_KEY-ICON_ID   = ICON_XLS.         " 아이콘
  G_FUNCTION_KEY-ICON_TEXT = 'SAMPLE다운'.      " 아이콘 옆에 나온는 텍스트
  G_FUNCTION_KEY-TEXT      = 'SAMPLE다운'.      " 마우스 오버했을 때 텍스트
  SSCRFIELDS-FUNCTXT_01    = G_FUNCTION_KEY.  " G_FUNCTION_KEY 행구조를 SSCRFIELDS-FUNCTXT_01 필드에 넣고있다.
ENDFORM.
*&---------------------------------------------------------------------*
...
*&---------------------------------------------------------------------*
FORM act_function_key .
  CASE SSCRFIELDS-UCOMM.       " SSCRFIELDS-UCOMM에는 사용자가 선택 화면에서 누른 버튼의 기능 코드가 저장.
    WHEN 'FC01'.               " 샘플 다운 버튼의 코드가 FC001이다.
      PERFORM EXCEL_DOWN_SMPL. " EXCEL_DOWN_SMPL 함수 호출
  ENDCASE.
ENDFORM.
*&---------------------------------------------------------------------*
*& Form GET_FILE_PATH
...
* METHOD 사용
  DATA : LT_FILE TYPE FILETABLE,    " 다이럴로그에서 선택한 파일 목록(여러 파일)의 경로와 정보가 담기는 내부 테이블.
         LS_FILE TYPE FILE_TABLE,   " LT_FILE 내부 테이블의 작업 영역으로, 하나의 파일 정보를 넣는다.
  ...
  CALL METHOD CL_GUI_FRONTEND_SERVICES=>FILE_OPEN_DIALOG
    CHANGING
      FILE_TABLE = LT_FILE      " 사용자가 파일 탐색기에서 선택한 파일들의 경로
      RC         = LV_RC.       " 열기(Open)를 눌렀으면 LV_RC에 1이상의 값이, 취소(Cancel) 0 저장.

  " 첫 번째 선택된 파일의 정보를 LT_FILE에서 읽어와 작업 영역 LS_FILE에 저장
...
FORM upload_from_excel .
  ...
  lv_numberofcolumns = 13.    " 여기 수정. 필드 개수로 수정
...
*& <--  p2        text
*&---------------------------------------------------------------------*
FORM excel_down_smpl .    " 템플릿 파일의 정보를 정의하고, 파일 경로를 조회한 다음, 실제 다운로드를 수행하고 결과 메시지 출력
...
* 파일 경로 조회
*   LS_KEY-OBJID를 인자로 넣어서 SET_DIRECTORY 함수 호출.
  PERFORM SET_DIRECTORY. " 팝업띄워서 저장할 경로 선택. 다운로드할 파일의 저장 위치(디렉토리) 및 기본 파일명을 설정.

* 엑셀 다운
  PERFORM DOWNLOAD_EXCEL_SMPL USING FNAME. " 엑셀 틀어서 파일 내용 채워주고 저장
...
*  CLEAR GV_DIRECTORY.
  ...
  " OBJFILE 객체의 DIRECTORY_BROWSE() 메소드 실행. (OBJFILE 변수는 ZABAP_08_TOP에 선언되어 있다.)

  OBJFILE->DIRECTORY_BROWSE( EXPORTING  INITIAL_FOLDER  = GV_INITIAL_DIR
                             CHANGING   SELECTED_FOLDER = GV_DIRECTORY    " GV_DIRECTORY는 선택한 파일의 경로
                             EXCEPTIONS CNTL_ERROR      = 1
                                        ERROR_NO_GUI    = 2
                                        NOT_SUPPORTED_BY_GUI = 3 ).
  IF SY-SUBRC = 0.
*    GV_FILE = GV_DIRECTORY && '\' && p_ls_key_objid && '.xlsx'.   " 전체 경로
  ELSE.
...
*  DESCRIBE TABLE TAB1 LINES RNUM.
...
*  CLEAR lt_clip.
  ...
  CONCATENATE 'MANDT' 'AGENCYNUM' 'NAME' 'STREET' 'POSTBOX'
              'POSTCODE' 'CITY' 'COUNTRY' 'REGION' 'TELEPHONE'
              'URL' 'LANGU' 'CURRENCY'
  ...
  LOOP AT TAB1.
    RNUM = sy-tabix + 1.
    CONCATENATE TAB1-MANDT    TAB1-AGENCYNUM TAB1-NAME    TAB1-STREET TAB1-POSTBOX
                TAB1-POSTCODE TAB1-CITY      TAB1-COUNTRY TAB1-REGION TAB1-TELEPHONE
                TAB1-URL      TAB1-LANGU     TAB1-CURRENCY
    INTO lt_clip SEPARATED BY lv_deli.
    APPEND lt_clip.
    CLEAR lt_clip.
  ENDLOOP.
...
*  PERFORM FILL_CELL USING GO_APPLICATION 01: 01 'SCARR 테이블 엑셀 샘플양식'.
*
*    CALL METHOD OF GO_APPLICATION 'Cells' = e_cell1
*    EXPORTING
*      #1 = 1
...
*
*
*  CALL METHOD OF e_range 'Merge'.

  DATA: font1 TYPE OLE2_OBJECT.

  GET PROPERTY OF e_range 'font' = font1.
  SET PROPERTY OF font1 'bold' = 1.
  SET PROPERTY OF font1 'size' = 20.

  CALL METHOD OF e_range 'select'.
  SET PROPERTY OF e_range 'HorizontalAlignment' = -4108.
...
FORM FILL_CELL  USING    PV_APPLICATION
  ...
  DATA: LV_ECELL TYPE OLE2_OBJECT.  " 셀 작업을 할 OLE2_OBJECT 를 생성

  " 받아온 전체 OLE 객체 PV_APPLICATION의 Cells 메소드를 호출한다.
  " 이때 인자로 PV_ROW, PV_COL을 넘긴다.
  " 결과는 LV_ECELL 객체에 담긴다.
...
FORM save_zsc_final  CHANGING GS_ZSC LIKE GS_ZSC.
  IF GS_ZSC-ZSTATUS = ICON_LED_YELLOW.            " 노란색 일때
...
FORM column_width  USING  p_column TYPE i
  ...
  " p_column를 첫 번째 파라미터로 넣어서 GO_SHEET 객체의 Columns 메소드 호출.
  " 리턴 값으로 OLE 객체(특정 열 객체)가lo_column에 할당.
  CALL METHOD OF GO_SHEET 'Columns' = lo_column
    EXPORTING
      #1 = p_column.

  " lo_column 객체의 select 메소드 호출.
  " 특정 범위나 객체를 선택(Selection) 상태로 만드는 역할. 열 전체가 파란색으로 하이라이트 되며 선택된 상태
  CALL METHOD OF lo_column 'select'.

  " go_applicatio 객체의 selection 메소드 호출.
  " 현재 활성화된 워크시트에서 선택되어 있는 영역을 반환한다. lo_selection 객체에 저장.
  CALL METHOD OF go_application 'selection' = lo_selection.



  " lo_column 객체의 ColumnWidth을 p_width로 지정한다.
  " ColumnWidth는 열의 너비를 지정하는 속성이다.
  " p_width의 값으로 열의 너비를 지정한다.
...
FORM column_format   USING  p_column TYPE i
  ...
  " lo_column 객체의 NumberFormatLocal 속성을 지정한다. 셀 서식을 넣을 수 있다.

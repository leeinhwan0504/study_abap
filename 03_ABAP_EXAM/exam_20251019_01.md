...
FORM data_retrieval.
  ...
  LOOP AT it_collect INTO wa_collect.         " it_collect 테이블 구조 변수를 반복
    ...
    " it_scarr 테이블에서 한 행을 읽어와서 wa_scarr에 넣는다.
    " 이때 조건에 it_scarr테이블은 STANDARD TABLE로 키가 없이 선언했기 때문에 WITH KEY로 사용한다.
    " carrid 와 wa_move의 carrid 가 같은 조건에 맞는 한 행을 읽어온다.
    READ TABLE it_scarr INTO wa_scarr
    WITH KEY carrid = wa_move-carrid.

    IF sy-subrc = 0.      " 읽어왔으면
      wa_move-carrname = wa_scarr-carrname.   " wa_move의 carrname 필드에 wa_scarr의 carrname의 값을 넣는다.
    ENDIF.


    " it_spfli 테이블에서 한 행을 읽어와서 wa_spfli에 넣는다.
    " 이때 조건에 it_spfli 테이블은 STANDARD TABLE로 키가 없이 선언했기 때문에 WITH KEY로 사용한다.
    " carrid와 wa_move의 carrid 가 같은 조건.
    " connid와 wa_move의 connid 가 같은 조건.
    READ TABLE it_spfli INTO wa_spfli
    WITH KEY carrid = wa_move-carrid
             connid = wa_move-connid.

    IF sy-subrc = 0.      " 읽어왔으면
      wa_move-cityfrom = wa_spfli-cityfrom.   " wa_move의 cityfrom 필드에 wa_spfli의 cityfrom의 값을 넣는다.
      wa_move-cityto   = wa_spfli-cityto.     " wa_move의 cityto 필드에 wa_spfli의 cityto의 값을 넣는다.
    ENDIF.
